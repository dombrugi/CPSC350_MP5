...
//constructor with default values of 1000 that increment accordingly
SimMenu::SimMenu(){
    studentID = 1000;
    facultyID = 1000;
}
...
//method that repeatedly simulates a user's interaction with the menu and simulates according to the number
void SimMenu::simulateMenu(){
    int input = 0;
    string menu;
    ...
    while(input != 11){
        ...
        switch (input){
            ...
            case 2:
                ...
                facultyMembers.printTreeInOrderFaculty();
                break;
            case 3:
                if(students.size() == 0){
            ...
            case 4:
                if(facultyMembers.size() == 0){
                    ...
                    bool checkFaculty;
                    checkFaculty = facultyMembers.printFacultyInfo(faculty);
            ...
            case 6:
                ...
                bool deletedStudent;
                deletedStudent = students.deleteStudent(deleteID, facultyMembers);
            ...
            case 8:
                if(facultyMembers.size() == 0){
                    ...
                    bool deletedFaculty;
                    deletedFaculty = facultyMembers.deleteFaculty(deleteIDFaculty);
            ...
            case 9:
                ...
                if(students.findNodeStudentTrue(userInput) == false){
                    ...
                    int changeID;
                    cin >> changeID;
                    if(facultyMembers.findNodeFaculty(changeID) == false){
                        cout << "That is not a valid advisor ID make sure it is correct and exists!" << endl;
                    } else {
                        students.changeStudentAdvisor(userInput, changeID, facultyMembers);
                    }
                }
                break;
            case 10:
                ...
                if(facultyMembers.findNodeFaculty(user) == false){
                    cout << "That is not a valid advisor ID make sure it is correct and exists!" << endl;
                } else {
                    ...
                    if(facultyMembers.removeAdvisee(user, removeID) == true){
                ...
                break;
            case 11:
                students.finalOutputFile(facultyMembers, students);
...
void SimMenu::addStudent(){
    if(facultyMembers.size() == 0){
        ...
        int advisorID;
        cin >> advisorID;
        if(facultyMembers.findNodeFaculty(advisorID) == false){
            ...
            students.insert(S1);
            facultyMembers.findNode(advisorID)->getData().getAdvisees()->addBack(studentID);
            students.checkDepths();
        }
    }
}
