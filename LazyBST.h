...
class LazyBST{
    ...
    private:
        ...
        int m_leftDepth;
        int m_rightDepth;
        void freeMemory(TreeNode<T>* currNode);
...
template <typename T>
void LazyBST<T>::freeMemory(TreeNode<T>* currNode){
    if(currNode == NULL){
        return;
    }
    freeMemory(currNode->m_left);
    freeMemory(currNode->m_right);
}

//method used to print student's info in order of ascending ID
template <typename T>
void LazyBST<T>::printTreeInOrderStudent(){
    printIOHelperStudent(m_root);
}

//helper method that checks if tree is empty, if not it traverses the tree in order and prints out student's info
template <typename T>
...
//method used to print faculty member's info in order of ascending ID
template <typename T>
void LazyBST<T>::printTreeInOrderFaculty(){
    printIOHelperFaculty(m_root);
}

//helper method that checks if tree is empty, if not it traverses the tree in order and prints out faculty member's info
template <typename T>
...
//method to find student given ID and print info
template <typename T>
...
//helper method that traverses tree and finds student. If student is not found returns false and vice versa
template <typename T>
...
//method to find faculty member given ID and print info
template <typename T>
...
//helper method that traverses tree and finds faculty member. If faculty member is not found returns false and vice versa
template <typename T>
...
//method to find faculty member given ID and delete the node
template <typename T>
bool LazyBST<T>::deleteFaculty(int data){
    bool result = deleteFacultyHelper(m_root, data);
}

//helper method for deleteing a faculty member that returns true if someone was deleted
template <typename T>
...
//method to find student given ID and delete the node
template <typename T>
...
//helper method that traverses the tree and removes student if ID is found, adjusting the order if necessary. Also return true or false to indicate if student was deleted
template <typename T>
...
//method that finds a the student object given ID and changes their advisor given the new advisor ID
template <typename T>
bool LazyBST<T>::changeStudentAdvisor(int data, int newID,  LazyBST<Faculty>& facultyMembers){ //fix this, doesn't remove from orginal advisor and add to new advisor
    bool result = ChangeStudentAdvisorHelper(m_root, data, newID, facultyMembers);
    return result;
}

//helper method that traverses and finds the student, changing the ID making sure the advisor exists
template <typename T>
...
// helper for creating the string of faculty members
template <typename T>
...
//helper for creating and returning the string of student's info 
template <typename T>
...
//method for creating the file with both faculty member and student's infoo
template<typename T>
...
//gets the right depth of a tree
template <typename T>
...
//helper method for the left depth of the tree
template <typename T>
...
//helper for the right depth of the tree
template <typename T>
...
void LazyBST<T>::checkDepths(){
    ...
    if((double)leftDepth / rightDepth > 1.5 || (double)rightDepth / leftDepth > 1.5){ //casted to doubles to avoid floating point exception
        rebalancedTree();
    }
}

//methods for rebalancing the tree, some methods I used help from chatGPT to make corrections more specifically the rebalancing
...
bool LazyBST<T>::contains(T data){ //no repeated values
...
void LazyBST<T>::insertHelper(TreeNode<T>*& currNode, TreeNode<T>* newNode){
    if(currNode == NULL){
        currNode = newNode; //pointer update is happening 
